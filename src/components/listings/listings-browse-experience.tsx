...
function ListingsFilterBar({ params }: { params: SearchListingsParams }) {
  return (
    <form
      action="/anunturi"
      className="mt-8 rounded-[1.5rem] border border-border/90 bg-card/90 p-4 shadow-[0_24px_70px_rgba(15,70,61,0.12)] backdrop-blur-md"
    >
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-[1.35fr_0.8fr_0.8fr_0.7fr_0.7fr_0.95fr_auto]">
        <label className="relative block">
          ...
          <input
            ...
            className="h-12 w-full rounded-full border border-border bg-card/80 pl-12 pr-4 text-sm text-brand-ink outline-none transition placeholder:text-brand-muted/75 focus:border-brand focus:ring-3 focus:ring-brand/20"
          />
        </label>
        ...
        <input
          ...
          className="h-12 rounded-full border border-border bg-card/80 px-4 text-sm outline-none focus:border-brand focus:ring-3 focus:ring-brand/20"
        />
        <input
          name="pret_max"
          inputMode="numeric"
  ...
  return (
    <label className="relative block">
      ...
      <select
        name={name}
        defaultValue={value}
        className={`h-12 w-full appearance-none rounded-full border border-border bg-card/80 ${
          icon ? "pl-11" : "pl-4"
        } pr-10 text-sm text-brand-ink outline-none transition focus:border-brand focus:ring-3 focus:ring-brand/20`}
      >
        {options.map((option) => (
  ...
  return (
    <form action="/anunturi" className="flex items-center gap-2">
      ...
      <select
        ...
        className="h-10 rounded-full border border-border bg-card/90 px-4 text-sm font-medium text-brand-ink outline-none focus:border-brand focus:ring-3 focus:ring-brand/20"
      >
...
function CompactListingRow({ listing }: { listing: Listing }) {
  ...
  return (
    <Link
      ...
      className="group grid gap-4 rounded-[1.25rem] border border-border/80 bg-card/92 p-3 shadow-[0_16px_46px_rgba(15,70,61,0.08)] transition hover:-translate-y-0.5 hover:shadow-[0_24px_64px_rgba(15,70,61,0.12)] sm:grid-cols-[220px_1fr_auto]"
    >
      <div
        className="relative aspect-[16/10] overflow-hidden rounded-[1rem] bg-brand-soft sm:aspect-auto sm:h-32"
      ...
      >
        ...
        <span className="absolute right-3 top-3 grid size-9 place-items-center rounded-full bg-card/90 text-brand shadow-sm">
...
export function ListingsMapPanel({
  ...
  return (
    <aside className="sticky top-24 hidden overflow-hidden rounded-[1.5rem] border border-border/80 bg-brand-soft shadow-[0_22px_70px_rgba(15,70,61,0.12)] xl:block">
      <div className="relative min-h-[520px] bg-brand-soft">
        <div
          aria-hidden="true"
          className="absolute inset-0 opacity-80 [background-image:radial-gradient(circle_at_20%_25%,rgba(0,95,63,0.14),transparent_14rem),radial-gradient(circle_at_80%_70%,rgba(233,180,76,0.13),transparent_13rem),linear-gradient(135deg,rgba(255,254,252,0.55),rgba(232,241,238,0.4))]"
        />
        <svg
          aria-hidden="true"
          viewBox="0 0 600 520"
        ...
        <div className="absolute left-5 top-5 flex h-12 w-[210px] items-center gap-3 rounded-full bg-card/95 px-4 text-sm font-semibold text-brand-ink shadow-sm">
