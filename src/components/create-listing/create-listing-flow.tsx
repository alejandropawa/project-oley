...
export function CreateListingFlow({
  ...
  if (showGuide) {
    return <PublishGuideScreen onContinue={closeGuide} />;
  }

  return (
    <div className="flex min-h-[calc(100svh-8.5rem)] w-full flex-col overflow-hidden rounded-lg border border-border bg-white shadow-soft">
      <header className="flex min-h-16 items-center justify-between gap-3 border-b border-border bg-white px-4 py-3 sm:px-6">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Button
            ...
            variant="outline"
            onClick={openGuide}
            aria-label="Deschide ghidul de publicare"
            className="h-10 rounded-full border-border bg-card px-4 text-sm font-semibold text-primary shadow-sm transition hover:border-primary/35 hover:bg-brand-soft"
          >
            <HelpCircle className="size-4" aria-hidden="true" />
            <span className="hidden sm:inline">Vezi ghidul</span>
            <span className="sm:hidden">Ghid</span>
          </Button>
          <Button
            asChild
            variant="ghost"
      ...
      <div className="grid flex-1 lg:min-h-0 lg:grid-cols-[17rem_1fr]">
        <div className="border-b border-border bg-brand-surface lg:min-h-0 lg:border-b-0 lg:border-r">
        ...
        <section className="flex min-h-0 flex-col bg-white">
          ...
          >
            ...
            {showAuthPrompt ? (
              <div className="mt-6 rounded-md border border-brand-border bg-brand-soft p-5 shadow-soft-sm">
                ...
                <div className="mt-5 grid gap-2 sm:grid-cols-3">
                  <Button
                    asChild
                    className="h-11 rounded-full bg-primary font-semibold text-primary-foreground"
                  >
                  ...
                  <Button
                    asChild
                    variant="outline"
                    className="h-11 rounded-full border-border bg-card font-semibold"
                  >
                  ...
                  <Button
                    ...
                    className="h-11 rounded-full font-semibold text-muted-foreground"
                  >
                    Continuă editarea
                  </Button>
                </div>
          ...
          <footer className="sticky bottom-0 z-10 border-t border-border bg-white/96 px-5 py-4 backdrop-blur sm:px-7 lg:static lg:backdrop-blur-none">
            <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
              <div className="hidden items-center justify-center gap-3 text-center lg:flex">
                ...
                <div className="text-left">
                  <p className="text-sm font-semibold text-foreground">
                    Anunțul va fi verificat după publicare
                  </p>
                  <p className="mt-0.5 text-xs font-semibold text-muted-foreground">
                    Ne asigurăm că anunțurile respectă regulile platformei.
                  </p>
                </div>
              </div>
              ...
              {step === 0 ? (
                <Button
                  asChild
                  variant="outline"
                  className="h-11 rounded-sm border-border bg-card px-6 font-semibold shadow-sm transition hover:border-primary/35 hover:bg-brand-soft hover:text-primary"
                >
                  <Link href="/">
                ...
                <Button
                  ...
                  className="h-11 rounded-sm border-border bg-card px-6 font-semibold text-foreground shadow-sm transition hover:border-primary/35 hover:bg-brand-soft hover:text-primary"
                >
              ...
              {step === 3 ? (
                <div className="flex flex-col items-stretch gap-2 sm:min-w-80 sm:items-end">
                  <Button
                    ...
                    disabled={isPublishing}
                    className={`${primaryActionButtonClassName} h-11 px-8 font-semibold`}
                  >
                    {isPublishing ? (
                      <Loader2
...
function getFirstInvalidStep(errors: CreateListingErrors): CreateListingStep {
  if (errors.type || errors.categorySlug) {
    return 0;
  }
